...
const handleMailStatusOrder = async data => {
    try {
        ...
        if (!user) return null
        await emailService.sendMail(user.email, 'Cập nhật trạng thái giao hàng tại Green Feast', `Đơn hàng của bạn với mã đơn hàng <b>${data.orderId}</b> đã được cập nhật trạng thái giao hàng thành <b>${data.status}</b>. Mọi vấn đề phát sinh vui lòng liên hệ hotline: 0123456789.`)
    }
    catch (err) {
